...
describe('#Service - test suite for core processing', () => {
  ...
  test('#createFileStream', () => {
    ...
    expect(result)
      .toStrictEqual(currentReadable)

    expect(fs.createReadStream)
      .toHaveBeenCalledWith(myFile)
  ...
  test('#getFileStream', async () => {
    ...
    expect(service.createFileStream)
      .toHaveBeenCalledWith(fileInfo.name)

    expect(service.getFileInfo)
      .toHaveBeenCalledWith(currentSong)
